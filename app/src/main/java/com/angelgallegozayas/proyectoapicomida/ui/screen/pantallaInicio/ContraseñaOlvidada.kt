...
@Composable
fun ContraseñaOlvidadaScreen(auth: AuthManager, navigateToLogin: () -> Unit) {
...
suspend fun forgotPassword(auth: AuthManager, email: String, context: Context) {
    if (email.isNotEmpty()) {
        ...
        Toast.makeText(context, "Ingrese su correo electronico", Toast.LENGTH_SHORT).show()
    }
}